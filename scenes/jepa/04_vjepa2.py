...
class VJEPA2Scene(Scene):
    def construct(self):
        ...
        subtitle = Text("Self-Supervised Video Models Enable Understanding, Prediction and Planning — Assran et al., FAIR at Meta, 2025", font_size=15, color=GRAY_B)
        ...
        phase1 = sized_box("Fase 1: V-JEPA 2 (livre de ação)\n+1 milhão de horas de vídeo da internet", font_size=15, color=WHITE, box_color=MECHANISM, box_opacity=0.3, min_width=7.0, min_height=1.1, line_spacing=1.2)
        phase2 = sized_box("Fase 2: condicionado por ação\npequena quantidade de trajetórias de robôs reais", font_size=15, color=WHITE, box_color=DECODER, box_opacity=0.3, min_width=7.0, min_height=1.1, line_spacing=1.2)
        ...
        state = sized_box("estado\natual", font_size=14, color=WHITE, box_color=ENCODER, box_opacity=0.35, min_width=1.8, min_height=1.0, line_spacing=1.1)
        state.shift(LEFT * 4.6)
        actions = VGroup(*[
            sized_box(f"ação {i+1}", font_size=13, color=WHITE, box_color=DECODER, box_opacity=0.3, min_width=1.4, min_height=0.7)
            for i in range(3)
        ]).arrange(DOWN, buff=0.25)
        futures = VGroup(*[
            sized_box(f"repr.\nfutura {i+1}", font_size=12, color=WHITE, box_color=MECHANISM, box_opacity=0.3 if i != 1 else 0.6, min_width=1.6, min_height=0.9, line_spacing=1.0)
            for i in range(3)
        ]).arrange(DOWN, buff=0.25)
        ...
        note1 = Text("nenhum pixel é gerado — só representações previstas, comparadas por custo desejado", font_size=15, color=MECHANISM)
