...
class FlamingoScene(Scene):
    def construct(self):
        ...
        lock1_fallback = Text("[fixo]", font_size=16, color=OLD).next_to(vision_box, UP, buff=0.1)
        ...
        lock2_fallback = Text("[fixo]", font_size=16, color=OLD).next_to(llm_box, UP, buff=0.1)
        ...
        variable_label = Text("features visuais\n(número variável)", font_size=16, color=GRAY_B, line_spacing=1.1).next_to(variable_feats, DOWN, buff=0.3)
        ...
        fixed_label = Text("tokens fixos", font_size=16, color=MECHANISM).next_to(fixed_feats, DOWN, buff=0.3)
        ...
        for name, color in zip(names, colors):
            b = sized_box(name, font_size=16, color=WHITE, box_color=color, box_opacity=0.3 if color == DECODER else 0.45, min_width=3.0, min_height=0.55)
            blocks.add(b)
        stack = VGroup(*blocks).arrange(DOWN, buff=0.12)
        assert_on_screen(stack, "flamingo gated stack")
        self.play(FadeIn(stack))

        tanh_note = Text("portão = tanh(parâmetro), inicializado ~0 -> LLM começa idêntico ao original", font_size=16, color=MECHANISM)
